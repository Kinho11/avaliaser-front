...
export const AdminProvider = ({ children }: IChildren) =>{
  ...
  const criarColaborador = async(userColaborador: IUserColaborador) =>{
    try {
      ...
      await API.post(`/auth/cadastrar-usuario?cargo=${userColaborador.cargo}`,userColaborador)
      navigate("/dashboard/admin")
      toast.success("Colaborador cadastrada com sucesso!", toastConfig);
  ...
  const pegarColaborador = async() =>{
    try {
      nProgress.start();
      API.defaults.headers.common["Authorization"] = token;
      const { data } = await API.get(`/auth/listar-usuarios?paginaQueEuQuero=0&tamanhoDeRegistrosPorPagina=10`)
